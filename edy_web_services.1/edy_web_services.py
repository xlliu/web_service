...
def generator_excel(version, pid, skip, limit):
    ...
    try:
        for v in dp:
            ...
            worksheet.write_row(n, 0, v.get("k_list"))
            n += 1
        workbook.close()
    except Exception, e:
        print e
...
def show_excel_info(version, pid, skip, limit):
    ...
    dpt_k = document_project.find_one({"版本": version},{"_id": 0, "k_list": 1}).skip(skip).limit(limit)
    ...
    dpt_value = []
    for dv in dpt_v:
        dpt_value.append(dv.get("k_list"))
    data_list = [dpt_k.get("k_list"), dpt_value]
    # data_list_1 = []
    # for dv in dpt_1:
    #     if "k_list" in dv:
